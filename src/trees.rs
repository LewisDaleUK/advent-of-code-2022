...
impl HasLineOfSight<i32> for Grid<i32> {
    fn lines_of_sight(&self, u_row: usize, u_col: usize) -> LineOfSight<i32> {
        ...
        LineOfSight {
			north: col.clone().collect_vec()[..u_row].into_iter().map(|i| **i).rev().collect_vec(),
			south: col.clone().collect_vec()[u_row+1..].into_iter().map(|i| **i).collect_vec(),
			east: row.clone().collect_vec()[u_col+1..].into_iter().map(|i| **i).collect_vec(),
			west: row.clone().collect_vec()[..u_col].into_iter().map(|i| **i).rev().collect_vec(),
		}
    }
}
...
impl Scenic for Grid<i32> {
	fn scene_score(&self, row: usize, col: usize) -> usize {
		if row == 0 || col == 0 || row == self.rows() -1 || col == self.cols() - 1 {
			...
			let los = self.lines_of_sight(row, col);
			los.north.scene_score(item) * los.south.scene_score(item) * los.east.scene_score(item) * los.west.scene_score(item)
		}
	}
...
pub fn construct(inputs: Vec<&str>) -> Grid<i32> {
	...
	for line in inputs.iter() {
		let row: Vec<i32> = line.chars().map(|c| c.to_string().parse::<i32>().unwrap()).collect();
		grid.push_row(row);
	}
	grid
}
...
mod tests {
    ...
    use super::{construct, HasLineOfSight, LineOfSight, Visible, Scenic};
	...
	fn it_tests_for_visiblity() {
		...
		let inputs = (1..grid.rows()-1)
			.flat_map(|row|
				(1..grid.cols()-1).map(|col| (row, col)).collect_vec()
			).collect_vec();
		...
		let result = inputs.iter().map(|(row, col)| grid.visible(*row, *col)).collect_vec();
		assert_eq!(expected, result);
	}
